...
MineWorld bridges a Godot 4 world editor with a headless MuJoCo physics authority over WebSocket, for simulation gameplay and teleoperation data capture. Docs are Chinese SSOT. POC + Hub（母港 · H12a–g）+ Portal + E1–E5 + W1 + R3 + H8–H12 + PL2 + IL-place 飞轮 Done. **Role：数聚球 3D 传送门前台** — `docs/21-ecosystem-federation.md`. **Hub 布局** — `docs/24-hub-mothership.md`（FakeMech；不上 Hub MuJoCo；L2 `floor2_walkable` + `hub_floor`/`hop_y` 视觉同步）。**公网 Demo 已通**：`playground.dev.databall.tech`（AWS ALB→WGateway→WG→腾讯 CVM）；`databall.cloud`（`docs/23-public-deploy.md`）为 ICP 后置，不阻塞 Phase A。**Now：A1–A3 + E9 + B1(`demo_race`) + B2(`duel_result`/B2.5 bot) + B3(房间模式 `solo|duel|shared_ffa`，duel 超员旁观) + Hub H8b/c + 棋牌室 Chess-P/P2/P3/P5（五对双人桌 + 扩容）+ 棋室聊天 Done**；**Next 候选 = HW-2.5 真机联调（HW-0 `hw_fake` / HW-1 键鼠前端 / HW-2 翻译层 Done）**（桌面键鼠 · 黑盒对接仓外 arm-bridge · `docs/28-hw-real-robot-plan.md`）/ E6–E7（blocked 待 PMS API）**。Changelog: `docs/19-changelog.md`. Platform: `docs/20-platform-portal.md`, `mw_platform/`. Identity map: `docs/22-identity-mapping.md`.

**Web 输入**：主站 **键鼠 only**；Web/Pico 摇杆已冻结放弃。勿恢复 DOM `mw_touch_pad` / 勿锁 `#canvas`。复盘见 `docs/29-web-pico-input-postmortem.md`。正式发版用 `scripts/deploy_playground.sh`。

## Project Structure & Module Organization

- `docs/` — design docs (`00` … `26-junqi-ssot.md`); `09-todo.md` execution; **`16` V-sprint**; **`18` Hub**; **`19` changelog**; **`20` portal**; **`21` 生态对接**; **`24` 母港布局**; **`25` 本地 Web 验收 + City 踩坑**; **`26` 军棋 SSOT**. 私有运维：`docs/ops.local.md`（**gitignore**，勿提交）。
