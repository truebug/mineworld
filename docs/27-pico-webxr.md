...
- **网络 ≠ 内存**：Pico 与桌面（常挂 VPN）到公网 CDN 的路径不同。已观测失败主因是 **下载中断 / 不完整文件 / nginx 对已是 gzip 的 `.spz` 再压缩**，**不是**已证实的头显 OOM。同域托管 `/xr/media/splats/` 是为对齐字节与链路，不是「Pico 只能开小场景」。大场景（如 sutro）应在设备上 A/B 后再下结论。
