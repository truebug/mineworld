# 18 · Hub 入口（母港 Hangar Core）

| 字段 | 值 |
|------|-----|
| **状态** | Active · H4–H11 Done；**布局叙事以 [24](24-hub-mothership.md) 为准**（H12a/b） |
| **日期** | 2026-07-21 |
| **关联** | [00](00-vision.md) · [15](15-course-correction.md) · [17](17-lobby-testfield.md) · [09](09-todo.md) · [19](19-changelog.md) · **[21](21-ecosystem-federation.md)** · **[24](24-hub-mothership.md)** |
| **主场景** | `godot/spike/demo_hub.tscn`（工程 `main_scene`） |
| **调试菜单** | `/?menu=1` → `demo_lobby.tscn`（文本选关，保留） |

> **Hub** = 默认起始关：**数聚球仿真宇宙母港 Hangar Core**（浮空岛核心接驳舱）。Godot 负责走动与叙事 UI；Gateway **Hub 房不接 MuJoCo**，只做多人在场同步（假物理位姿）。  
> **真数据价值**仍在进门之后的本仓 MuJoCo 遥操关（车间 / 街区等）。  
> **展厅/教室等**不在本仓仿真：经展柜通道进入 PMS Space（对接，见 [21](21-ecosystem-federation.md)）。  
> **三类出口空间分区**（本仓 / PMS 卡片 / 边缘）见 [24](24-hub-mothership.md)。

---

## 1. 背景故事（冻结切片）

飞行员抵达 **母港 Hangar Core** —— 浮空岛/舰队母港的核心大厅。  
厅内按翼区分三类出口；中央电梯可达 L2 观景廊。大厅本身不是竞技场，只是集合与分流；外延网格甲板为后续建筑群留白。

| UI 区 | 作用 |
|-------|------|
| 左侧文本 | Lore / 操作提示 / 当前门说明（中文优先双语） |
| 右侧导引图 | 顶视小地图（厅轮廓 + 自己/他人点） |
| 右上人物卡 | 本地昵称 / 简卡（Portal 登录后带 profile） |
...
| 门 | 路线 | 翼 / 类型 | 本期 | 备注 |
|----|------|-----------|------|------|
| **A · 仿真工坊** | 单人精细操作 / 自定义关节 · IL | 东 · A 本仓 | ✅ 可进 | → `demo_workshop` + MuJoCo |
| **B · 机甲训练场** | 多人联网巡航 / 推箱 | 西 · A 本仓 | ✅ 可进 | → `demo_city`；共享房 `city`（最多 5 人） |
| **C · 设计室** | 自定义空间 / 契约导出 | 北 · B 卡片 | 后置 | 编辑器 + T4.2 |
| **D · 边缘任务坞** | 真机/边缘入口 | 西北 · C 边缘 | 后置 | 叙事占位；不透传真机 |
| **E · 竞技场** | 机甲格斗（1v1 / 团队对战）· 组队排名 | 南 · A 壳 | 建设中 | 门 E 占位（不可进）+ F 四态 stub；格斗权威后置 |
| **R · 赛车场** | 多人计时竞速 | 东南 · A 本仓 | ✅ 可进 | → `demo_race`；共享房 `race`（最多 6 人） |
| **展柜 / 房间通道** | PMS Space 卡片 | 北翼 | **E4 stub** | F → 新标签 URL |
