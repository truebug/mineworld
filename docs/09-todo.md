...
> Next 候选 = **Splat-P1d（看/走提频 pose，静止降频）** / 程序星空垫层（可选）/ **HW-2.5 真机联调** / E6–E7（blocked 待 PMS API）。  
> 交接 SSOT：[35-splat-render-handover.md](35-splat-render-handover.md)。  
...
| Skin-S1 | 皮肤烘焙脚本 | `scripts/skin_bake.py`：PLY→SPZ→质检→occupancy；igs0047 重 bake 与现网一致 | [ ] 下一批 · [38](38-asset-generation-plan.md) |
| Asset-N1 | 资产管线批次1 | `fetch_cc0_assets.py` + 棋牌室家具/工坊 PBR 接线；pck 63→31MB | [x] 2026-08-21 两轮 review 闭环 |
...
| Fun-R | 邀请链接 | `?room=xxx` 私密房码，复制链接拉人开黑 | [x] mw/invite_link.gd 按钮 + 房码复制/跳转 · 2026-08-13 |
...
| ID | 任务 | 验收 | 状态 |
|----|------|------|------|
| H8 | 可乘电梯 + 可上 L2 | 轿厢或瞬移；栏杆碰撞；小地图层 | [x] 薄乘 + `hub_floor` 同步 |
| H8b | L2 甲板空气墙 | `floor2_walkable`；二楼不能走出甲板悬空 | [x] |
| H8c | Hub 空格跳跃互见 | Space；`hop_y` 进 state；玩法关不占用 | [x] playground `20260723-181418` |
...
| H12g | 装饰球体/圆顶/球仓 | 舱顶圆顶、落地储罐、球仓；无悬空细环 | [x] v0′ |
