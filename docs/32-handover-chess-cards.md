...
21 点已升级为**双座多手牌共享庄家**（轮转行动、旁观入座、禁止局中重置）；五对支持**人机**（5 秒无人加入自动匹配 AI 执红）并新增**显式 5 秒倒计时**（`ai_fill_at` 字段 + 客户端倒计时标签）；五对牌堆唯一性 bug（4×54 误建 + JOKER 身份歧义）、手牌溢出桌布 UI 均已修复；blackjack/wudui 牌桌已放大（felt 上限 920×520）。线上 build `20260809-094718`（二批发版）。
