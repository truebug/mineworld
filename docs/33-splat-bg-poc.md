...
| ~~P1~~ ✅ | shell 注入：`?splat=lab3&level=demo_race` → `splat_bg.js` 垫底 canvas + Spark 加载；vendored three/Spark（import map，无构建无 CDN）；`mw/splat_bridge.gd` 透明合成 + `MW_CAM_POSE` 20Hz 推送 | 硬刷可见背景，关开关零注入 · 2026-08-14 |
