...
func _refresh_board_from_authority() -> void:
	...
	if _title_label != null:
		if vs_ai:
			_title_label.text = MWi18n.t(
				"五子棋 · 人机（F/Esc 起身）",
				"Gomoku vs AI (F/Esc to stand)"
			)
		else:
			_title_label.text = MWi18n.t(
				"五子棋 · 人对人（F/Esc 起身）",
				"Gomoku PvP (F/Esc to stand)"
			)
	...
	if status == "finished":
		if winner == GomokuScript.BLACK:
			_set_status(MWi18n.t("黑胜 · 按 F 再来", "Black wins · F rematch"))
		elif winner == GomokuScript.WHITE:
			_set_status(MWi18n.t("白胜 · 按 F 再来", "White wins · F rematch"))
		elif bool(d.get("full", false)):
			_set_status(MWi18n.t("平局 · 按 F 再来", "Draw · F rematch"))
		else:
			_set_status(MWi18n.t("终局 · 按 F 再来", "Over · F rematch"))
...
func _draw_board() -> void:
	...
	var cells: Array = _view_detail().get("cells", [])
	for y in s:
		for x in s:
			var idx := y * s + x
			var v := 0
